...
//            self.unusedMethods = CleanUnusedMethods().find(path: self.selectedPath)
    ...
    @IBAction func onOpenFIleClicked(_ sender: NSButton) {
        let openPanel = NSOpenPanel.init()
        openPanel.treatsFilePackagesAsDirectories = true
        openPanel.allowsMultipleSelection = true
        openPanel.canChooseDirectories = true
        openPanel.canChooseFiles = true
        openPanel.beginSheetModal(for: NSApplication.shared.keyWindow!) { (result:NSApplication.ModalResponse) in
            if result != NSApplication.ModalResponse.OK {
                return
            }
            openPanel.orderOut(self)
            self.dragFileOk(filePath: openPanel.url?.absoluteString ?? "")
        }
    }
}
